...
- **Routing** — file-based under `frontend/app/`. Dynamic pages: `app/products/[category]`, `app/products/[category]/[variant]`, `app/sectors/[sector]`; hub pages exist at `/products` and `/sectors`; `/products/wider-seafood-range` is a static page (takes precedence over `[category]`) describing the partner-sourced shellfish/whole-fish/fillets lines from `WIDER_RANGE` in `products.js` — deliberately format-level only, no species claims, until the business fixes the range. `app/sitemap.js` and `app/robots.js` are generated from the data files — never hand-edit a sitemap. `app/admin/layout.jsx` sets `robots: noindex`.
